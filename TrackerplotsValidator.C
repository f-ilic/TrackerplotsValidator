...
TObject* Validator::find_plot_in_file(TDirectory* baseDir, string file_name_to_find) {
    ...
    cout << "[SHOULD NEVER REACH THIS]" << endl;
    return 0;
}
...
Validator::Validator(const TGWindow *p, UInt_t w, UInt_t h) {
    ...
    plot_btn->Connect("Clicked()", "Validator", this, "tmp_f()");
...
// returns 0 ... 100
// Similarity in percent, based on intersection:
//
...
int TrackerplotsValidator() {
    new Validator(gClient->GetRoot(), 200, 200);
    return 0;
}


void Validator::tmp_f() {
...
//       cout << "[FAIL] Opening File" << endl;
   ...
   TCanvas* resultCanvas = new TCanvas("Result Canvas", "", 800, 800);
   ...
   for(auto& e : plots){
      ...
      ((TH1*)e.second)->SetMarkerStyle(2);
        ...
        ((TH1*)e.first)->SetStats(true);
//        ((TH1*)e.second)->SetStats(true);
      ...
      cout << " with " << similarity  << "% intersection similarity \t" << e.first->GetTitle() << endl;
